...
if( ! function_exists( 'elito_excludecat' ) ) {
  function elito_excludecat($query) {
  	if ( $query->is_home ) {
  ...
  add_filter('pre_get_posts', 'elito_excludecat');
}
...
class Elito_Excerpt {
  ...
  public static function length($new_length = 55) {
    Elito_Excerpt::$length = $new_length;
    add_filter('excerpt_length', 'Elito_Excerpt::new_length');
    Elito_Excerpt::output();
  }

  // Tells WP the new length
  public static function new_length() {
    if( isset(Elito_Excerpt::$types[Elito_Excerpt::$length]) )
      return Elito_Excerpt::$types[Elito_Excerpt::$length];
    else
      return Elito_Excerpt::$length;
  }
...
if( ! function_exists( 'elito_excerpt' ) ) {
  function elito_excerpt($length = 55) {
    Elito_Excerpt::length($length);
  }
}

if ( ! function_exists( 'elito_new_excerpt_more' ) ) {
  function elito_new_excerpt_more( $more ) {
    return ' ';
  }
  add_filter('excerpt_more', 'elito_new_excerpt_more');
}

/* Tag Cloud Widget - Remove Inline Font Size */
if( ! function_exists( 'elito_tag_cloud' ) ) {
  function elito_tag_cloud($tag_string){
    return preg_replace("/style='font-size:.+pt;'/", '', $tag_string);
  }
  add_filter('wp_generate_tag_cloud', 'elito_tag_cloud', 10, 3);
}

/* Password Form */
if( ! function_exists( 'elito_password_form' ) ) {
  function elito_password_form( $output ) {
    $output = str_replace( 'type="submit"', 'type="submit" class=""', $output );
    return $output;
  }
  add_filter('the_password_form' , 'elito_password_form');
}


/* Widget Layouts */
if ( ! function_exists( 'elito_footer_widgets' ) ) {
  function elito_footer_widgets() {
...
/* WP Link Pages */
if ( ! function_exists( 'elito_wp_link_pages' ) ) {
  function elito_wp_link_pages() {
    $defaults = array(
      'before'           => '<div class="wp-link-pages">' . esc_html__( 'Pages:', 'elito' ),
      'after'            => '</div>',
      'link_before'      => '<span>',
      'link_after'       => '</span>',
...
/* Metas */
if ( ! function_exists( 'elito_post_metas' ) ) {
  function elito_post_metas() {
  ...
  <div class="bp-top-meta">
    ...
    <div class="bp-author">
      <?php
      printf(
        '<span>'. esc_html__('by','elito') .' <a href="%1$s" rel="author">%2$s</a></span>',
...
/* Author Info */
if ( ! function_exists( 'elito_author_info' ) ) {
  function elito_author_info() {
    ...
    $author_text = $author_text ? $author_text : esc_html__( 'Author', 'elito' );
...
=============================================== */
if ( ! function_exists( 'elito_comment_modification' ) ) {
  function elito_comment_modification($comment, $args, $depth) {
  ...
  <<?php echo esc_attr($tag); ?> <?php comment_class('item ' . $comment_class .' ' ); ?> id="comment-<?php comment_ID() ?>">
   ...
   <article>
    <div id="div-comment-<?php comment_ID() ?>" class="elito-comment">
    <?php endif; ?>
    ...
    <div class="comment-main-area">
      ...
      <em class="comment-awaiting-moderation"><?php echo esc_html__( 'Your comment is awaiting moderation.', 'elito' ); ?></em>
      <?php endif; ?>
      <div class="comment-content">
        ...
        <div class="comments-reply">
          <?php
            ...
            'reply_text' => '<span class="comment-reply-link icofont icofont-reply-all">'.esc_html__( 'Reply', 'elito' ).'</span>',
            'before' => '',
            'class'  => '',
            'depth' => $depth,
...
if( ! function_exists( 'elito_move_comment_field' ) ) {
  add_filter( 'comment_form_fields', 'elito_move_comment_field' );
  function elito_move_comment_field( $fields ) {
...
/* Title Area */
if ( ! function_exists( 'elito_title_area' ) ) {
  function elito_title_area() {
    ...
    if ($elito_meta && (!is_archive() || is_woocommerce_shop())) {
      $custom_title = $elito_meta['page_custom_title'];
      if ($custom_title) {
    ...
    if ( is_home() && is_front_page() ) {
      echo esc_html__( 'Blog', 'elito' );
    } elseif ( is_home() && !is_front_page() ) {
      single_post_title();
    } elseif ( is_search() ) {
      printf( esc_html__( 'Search Results for %s', 'elito' ), '<span>' . get_search_query() . '</span>' );
    } elseif ( is_category() || is_tax() ){
      single_cat_title();
    } elseif ( is_tag() ){
      single_tag_title( esc_html__('Posts Tagged: ', 'elito') );
    } elseif ( is_archive() ){
      if ( is_day() ) {
        printf( wp_kses( __( 'Archive for <span>%s</span>', 'elito' ), array( 'span' => array() ) ), get_the_date());
      } elseif ( is_month() ) {
        printf( wp_kses( __( 'Archive for <span>%s</span>', 'elito' ), array( 'span' => array() )), get_the_date( 'F, Y' ));
      } elseif ( is_year() ) {
        printf( wp_kses( __( 'Archive for <span>%s</span>', 'elito' ), array( 'span' => array() ) ), get_the_date( 'Y' ));
      } elseif ( is_author() ) {
        printf( wp_kses( __( 'Posts by: <span>%s</span>', 'elito' ), array( 'span' => array() ) ), get_the_author_meta( 'display_name', $wp_query->post->post_author ));
      } elseif( is_woocommerce_shop() ) {
        esc_html_e( 'Shop', 'elito' );
      } elseif( is_product() ) {
        esc_html_e( 'Shop Single', 'elito' );
      } elseif ( is_post_type_archive() ) {
        post_type_archive_title();
      } else {
        esc_html_e( 'Archives', 'elito' );
      }
...
// Pingback Head
function elito_pingback_header() {
  if ( is_singular() && pings_open() ) {
    printf( '<link rel="pingback" href="%s">' . "\n", get_bloginfo( 'pingback_url' ) );
  }
}
add_action( 'wp_head', 'elito_pingback_header' );


/**
 * blog pagination function.
 */
if ( ! function_exists( 'elito_posts_navigation' ) ) :
  function elito_posts_navigation() {
 ...
 */
if ( ! function_exists( 'elito_single_pagination' ) ) :
  function elito_single_pagination() {
    $newer_post = cs_get_option('newer_post');
    $newer_post = ( $newer_post ) ? $newer_post : esc_html__('Next Post', 'elito');
    $older_post = cs_get_option('older_post');
    $older_post = ( $older_post ) ? $older_post : esc_html__('Previous Post', 'elito');
    the_post_navigation(
      array(
        'next_text' => '<span class="meta-nav" aria-hidden="true">' . esc_html( 'Next Post', 'elito' ) . '</span> ' .
          '<span class="screen-reader-text">' . esc_html( $newer_post ) . '</span> <br/>' .
          '<span class="post-title">%title</span>',
        'prev_text' => '<span class="meta-nav" aria-hidden="true">' . esc_html( 'Previous Post', 'elito' ) . '</span> ' .
